...
const props = withDefaults(defineProps<ITimeseries>(), {
    ...
    iDs: () => [], // gis
    data: () => [], // 数据
    formatter: (date: string) => dayjs(date).format(minuteFormat03),
    loading: false,
    animationFrequency: 300,
})
...
const formatter = () => {
    if (props.time.length > 0) {
        return props.formatter(props.time[current.value])
...
const step = () => {
    const frame = () => {
        // console.log('animation control step changed>>>', current.value, props.loading)
        if (current.value >= props.time.length - 1 || props.loading) {
            stop()
        } else {
    ...
    }, props.animationFrequency)
}

const formatCommon = (params: string): string => {
    if (params !== undefined && params !== '') {
        return props.formatter(params)
...
watch(
    ...
    (newVal, oldVal) => {
        if (!isEmpty(newVal) && newVal !== oldVal) {
            // console.log('animation control :>> ', newVal, oldVal, newVal === oldVal)
            stop()
            current.value = 0
            sliderMax.value = newVal.length === 0 ? 1 : newVal.length - 1
            console.log('animation control time changed>>>', sliderMax.value, props.loading)
            range.length = 0
            range.push(...[formatCommon(newVal[0]), formatCommon(newVal.at(-1)!)])
...
watch(
    ...
    (newVal) => {
        ...
        // console.log('props.loading>>>', props.loading, isDisabled.value)
    },
    {
        immediate: true,
    },
...
<template>
    <div class="animation-control-main">
        ...
        <div class="speed">
            ...
            <a-select
                :trigger-props="{ contentClass: 'select-speed-popup-content' }"
                class="select-speed"
                v-model="speedType"
                :disabled="isDisabled"
                size="mini"
