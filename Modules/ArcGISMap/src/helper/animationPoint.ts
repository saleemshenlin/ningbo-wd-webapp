...
import BaseLayerViewGL2D from '@geoscene/core/views/2d/layers/BaseLayerViewGL2D'

export const AnimationPointLayer = (
    size = 70,
    colors = '0.23, 0.43, 0.70',
    hitBuffer = 10,
): any => {
    const CustomLayerView2D = (BaseLayerViewGL2D as any).createSubclass({
       ...
       void main(void) {
        ...
        hitTest: async function (_mapPoint: Point, screenPoint: Point) {
            // Coordinates of the clicked screen point.
            const { x, y } = screenPoint

            // The map view.
            const view: MapView = this.view
            ...
            const distances: any = this.layer.graphics.map((graphic: Graphic) => {
                const graphicPoint = view.toScreen(graphic.geometry as Point)
                return Math.sqrt(
            ...
            distances.forEach((distance: number, i: number) => {
            ...
            return await Promise.resolve([graphicHit])
        },

        // Called internally from render().
        updatePositions: function (renderParameters: any) {
            const gl = renderParameters.context
            const stationary: boolean = renderParameters.stationary
            const state = renderParameters.state
    ...
    return (GraphicsLayer as any).createSubclass({
        createLayerView: function (view: MapView) {
