...
import {
    ...
    defaultBaseMapUrl,
    defaultLODs,
} from '../../helper/ufMapHelper'
import { tmapTileInfo, tmapWTileInfo, TMapLayer } from '../../helper/tmapLayer'
import _, { isEmpty } from 'lodash'
import { PNGMapLayer } from '../../helper/pngLayer'
import { MODEL_PIPE } from '../../store/const'
            ...
            if (baseMap.wkid === 4490) {
                return [
                    new TMapLayer({
                        ...
                        opacity: baseMap.opacity,
                        tk: baseMap.tMapTK,
                        tileInfo: baseMap.wkid === 4490 ? tmapTileInfo : tmapWTileInfo,
                    }),
                    new TMapLayer({
                        urlTemplate: baseMap.baseUrl
                            .replaceAll('vec', 'cva')
                ...
                return [
                    new WebTileLayer({
                        urlTemplate: `${baseMap.baseUrl}&tk=${baseMap.tMapTK}`,
                        subDomains: ['t0', 't1', 't2', 't3', 't4', 't5', 't6', 't7'],
                        id: 'baseLayer',
                        opacity: baseMap.opacity,
                    }),
                    new WebTileLayer({
                        urlTemplate: `${baseMap.baseUrl
                            .replaceAll('vec', 'cva')
                            .replaceAll('img', 'cva')}&tk=${baseMap.tMapTK}`,
                        subDomains: ['t0', 't1', 't2', 't3', 't4', 't5', 't6', 't7'],
                        id: 'baseLayer-label',
                        opacity: baseMap.opacity,
                    }),
...
const renderBasicLayers = () => {
    ...
    if (baseGeoMap.basicGIS !== null && view !== null && !isEmpty(baseGeoMap.basicGIS.mesh)) {
...
watch(
    ...
    async (dataSet) => {
        console.log('map 触发数据源 >>>', dataSet, props.resultMap.index, props.resultMap.legend)
        if (dataSet === null) {
            loading.value = false
            return
        }
        ...
        if (props.resultMap.resultGIS !== null && view !== null && renderFlag) {
            loading.value = true
            console.time('dataSet :>> updateResultSet')
            renderResultLayer(
                dataSet.key ?? MODEL_PIPE,
                map,
                props.resultMap.resultGIS!,
                dataSet.time,
                props.resultMap.legend,
                view,
                () => {
                    loading.value = false
                    console.timeEnd('dataSet :>> updateResultSet')
                },
            )
        } else {
            loading.value = false
