...
interface IProps {
    ...
    baseMap: {
        ...
        symbolMap?: Record<string, string> // 图标配置
    }
    /** 地图Popup */
    popup: {
...
const changeLayers = (layers: string[]) => {
    const tempArray = [...layers]

    if (!tempArray.includes(WDMapConst.BASE_CLOSING_VALVE)) {
        tempArray.push(WDMapConst.BASE_CLOSING_VALVE)
    }

    if (!tempArray.includes(WDMapConst.BASE_HIGHLIGHT_PIPE)) {
        tempArray.push(WDMapConst.BASE_HIGHLIGHT_PIPE)
    }

    mapLayerControl.value = tempArray
}
// #endregion
    ...
    <div class="map-panel">
        <DhiDssWDMap
            ...
            :baseGeoMap="{
                ...
                symbolMap: props.baseMap.symbolMap,
            }"
            :result-map="{
